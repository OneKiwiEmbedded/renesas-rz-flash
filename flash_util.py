...
FLASH_WRITER_FILE = "Flash_Writer_SCIF_RZV2L_SMARC_PMIC_DDR4_2GB_1PCS.mot"
BL2_FILE = "bl2_bp-onekiwi-rzv2l_pmic.srec"
FIP_FILE = "fip-onekiwi-rzv2l_pmic.srec"
...
class FlashUtil:
    def __init__(self):
        self.flash_type = 0
        self.serial = ''
        self.flash_writer_image = FLASH_WRITER_FILE
        self.bl2_image = BL2_FILE
        self.fip_image = FIP_FILE

        argparser = self.argparse_defaults()
        self.handle_path_overrides()

        if self.flash_type == 0:
            print("Please set flash spi or mmc")
            print("./flash.py --spi")
            print("./flash.py --mmc")
            return
    ...
    def write_bootloader(self):
        with alive_bar(4, title='Flashing', bar='halloween', spinner='notes', enrich_print=False) as bar:
    ...
    def change_baudrate(self):
        ...
        self.write_serial_cmd("SUP", prefix="\r")
    ...
    def flash_erase_qspi(self, bar):
        ...
        self.wait_for_serial_read("Clear OK?")
